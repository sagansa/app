...
<div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
    <x-input.image name="image" label="Image">
        <div
            x-data="imageViewer('{{ $editing && $salesOrderOnline->image ? \Storage::url($salesOrderOnline->image) : '' }}')"
    ...
    >
        ...
        <option disabled {{ empty($selected) ? 'selected' : '' }}>Please select the Delivery Service</option>
    ...
    <x-input.text
        name="receipt_no"
        label="Receipt No"
        value="{{ old('receipt_no', ($editing ? $salesOrderOnline->receipt_no : '')) }}"
        maxlength="255"
    ></x-input.text>
    ...
    <x-input.date
        name="date"
        label="Date"
        value="{{ old('date', ($editing ? optional($salesOrderOnline->date)->format('Y-m-d') : 'today()')) }}"
        required
    ></x-input.date>
    ...
    <x-input.textarea name="notes" label="Notes" maxlength="255"
        >{{ old('notes', ($editing ? $salesOrderOnline->notes : ''))
        }}</x-input.textarea
    >
